# 🛸 RHmachine

On-chain market radar · discover · track · analyse

[Keyboard controls and shortcuts](KEYBOARD.md) — tabs, Split Radar, scrolling, `d` DexScreener, `i` analysis, `o` saved charts, chat and Settings.

## Set up with your AI

Give your assistant this link:

**https://github.com/sdb001/RHmachine-setup/blob/main/SETUP.md**

> Help me install RHmachine and connect it to my AI using these official instructions. Keep automatic analysis off and let me enter credentials locally.

**Release status: unsigned beta.** [Download for Apple Silicon macOS](https://github.com/sdb001/RHmachine-beta/releases/tag/v2.1.0-beta.2). No GitHub invitation is required. Application source remains private. This is a test build, not a stable release.

This repository contains public setup instructions. Application source and private development data are not distributed here.

Preview checks passed: latest research/dashboard integration, neutral AI identity, isolated local setup, Hermes configuration preservation, MCP discovery, installer integrity and protection against replacing an existing command. Apple signing/notarization is not included in this beta. Real-provider acceptance testing is still pending.
